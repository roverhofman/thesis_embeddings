...
def ts_to_rp(ts: np.ndarray, image_size: int | None = None) -> np.ndarray:
    """
    Compute the raw Recurrence Plot (RP) array for a 1D time series.
    """
    ts = np.array(ts).reshape(1, -1)
    rp = RecurrencePlot()
    return rp.transform(ts)[0]
...
def rp_images(
    ...
    """
    Convert each row of train/val/test DataFrames into normalized RGB RP images.
    Returns three lists of PIL.Image objects of size (image_size, image_size).
    """
    raw_train = [ts_to_rp(row, image_size) for row in train_df.values]
    raw_valid = [ts_to_rp(row, image_size) for row in valid_df.values]
    raw_test  = [ts_to_rp(row, image_size) for row in test_df.values]
